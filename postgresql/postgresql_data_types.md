...
* Opciones: `int GENERATED BY DEFAULT AS IDENTITY`, `serial`, ... 
...
* Opciones: `double precision` y sus alias: `float`, `float8`, `numeric` y sus alias: `decimal`, `real` y sus alias `float4`
...
Opciones: `bigint` (`int8`), smallint (`int2`), integer (`int`, `int4`)
* Nuestra opción por defecto: `int`



## Textos

* Opciones: `character varying(n)` (alias `varchar(n)`, `character(n)` (alias `char(n)`), `text`
...
Si hay que limitar por lógica de negocio el número de caracteres de un campo habitualmente esto debería venir acompañado de otro tipo de validación (por ejemplo que un _id_ esté construido de una determinada forma, o tenga un número mínimo de caracteres, ...) Para ello se pueden usar `TRIGGER`, `CONSTRAINT`, o `DOMAIN`. Es un poco más de trabajo pero también más fiable.

Debe tenerse en cuenta que limitar el texto que un usuario puede entrar [siempre es una buena idea](http://highscalability.com/blog/2013/8/26/reddit-lessons-learned-from-mistakes-made-scaling-to-1-billi.html) o "alguien encontrará la forma de subir un campo de 5GB de texto", y si no se controla en otra capa o mediante otra técnica un `varchar(10000)` puede ser una forma de hacerlo.
...
insert into test_varchar (v) values ('1234567'::varchar);
> ERROR:  22001: value too long for type character varying(6)
insert into test_varchar (v) values ('123456 '::varchar); /* Los trailing spaces son truncados automáticamente cuando pasan del límite. Quizás lo que quieres quizás no
> INSERT 0 1
```
...
En ocasiones es de interés almacenar en la base de imágenes, ficheros o [strings binarios](https://www.postgresql.org/docs/9.1/datatype-binary.html). Para estos casos se usa `bytea`. PostgreSQL no soporta el estándar `blob` pero [bytea es en esencia lo mismo](http://www.postgresqltutorial.com/postgresql-python/blob/).
